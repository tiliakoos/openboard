...
final class BoardController: ObservableObject {
    ...
    private func handle(_ event: HookServer.Event) async {
        ...
        */
        ...
        /*
         A surface the board is not listening to gets no key, and neither does a
         headless probe — `claude` driven in a pty with no human behind it (CodexBar's
         usage probe does this every 16 minutes), which reports `cli` like any pty
         session and is only told apart by its parent chain.
